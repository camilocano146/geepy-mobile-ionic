...
@Component({
  selector: 'app-e-sims',
  templateUrl: './my-e-sims-page.component.html',
  styleUrls: ['./my-e-sims-page.component.scss'],
})
export class MyESimsPage implements OnInit {
  /**
   * Ususario de la app
   */
  public user: User;
  ...
  ionViewDidEnter1(eventInfiniteScroll?: CustomEvent){
    if (this.nextPage) {
      this.simCardService.getE_SimCardVoyager(this.user.id, this.pageSim++ * this.limit, this.limit, this.auxText).subscribe(res => {
        console.log(res);
  ...
  openPhotoViewer() {
    this.photoViewer.show('../../../assets/img/simcard.svg');
  }
}
